# About the Project
1. Jsoup Library Utilization: The crawler leverages the Jsoup library to extract images from specified URLs, parsing the HTML effectively.
2. Subpage Navigation: It navigates to and processes subpages of the target website to ensure thorough image extraction.
3. Depth Control: The depth of search is configurable, with the current default set at two levels deep.
4. Concurrent Processing: Utilizes Java's Thread class for multithreading, allowing simultaneous processing of multiple pages.
5. Frontend Display: Features a simple frontend interface that presents the crawling results to the user in a user-friendly manner.
6. Rate Limiting: Implements a delay mechanism between requests to mitigate the risk of overloading the target server and to comply with rate-limiting policies.
7. Potential for Logo Identification: While not currently implemented, the framework is set up to allow for future integration of a machine learning model to identify website logos.
8. Architecture Design: The crawling logic and threading are encapsulated within the JsoupWebCrawler class, while the servlet class manages URL input and delegates to the WebCrawler interface implemented in JsoupWebCrawler.java.

Given the project timeline, I have diligently worked to implement a comprehensive set of features that provide a robust foundation for the web crawler. Future enhancements and optimizations can be integrated as needed. Feedback and contributions are always welcome to improve the tool's functionality and performance.
